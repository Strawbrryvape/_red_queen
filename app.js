...
"the five required sections.",
  ...
  (function ensureModelTestButton() {
    try {
      ...
      [
        ...
        ["artifactsToggle", "rq_artifacts", "ARTIFACT EXPORT",
         "Turn a round into a downloadable file. VERIFIED gives one file (the speaking seat verbatim + a deterministic signature block). DIVIDED-PARALLEL gives one file PER SEAT, never merged. Nothing is generated by the export \u2014 every word is a seat\u2019s own. The verdict tag rides in the filename and the first line. window.__rqExport() or __rqExportDocx().",
         "Rounds stay in the browser; there is no file export."],
        ...
        ["courierProxyToggle", "rq_courier_proxy", "COURIER: READ PROXY",
         "\u26A0 SENDS URLs TO A THIRD PARTY. When a host refuses a direct read, retry through r.jina.ai, a public text extractor. It works on most of the web that CORS blocks. Cost: they see which URLs you fetch. No API key is involved and it reads public pages only \u2014 a privacy cost, not a key-exposure one. Receipts record via=proxy so proxied text is never mistaken for a direct read.",
         "Blocked hosts stay blocked; paste them with window.__rqPaste(url, text)."],
  ...
  function renderDividedPanel(answers) {
    ...
    answers.forEach((a) => {
      ...
      h.appendChild(w);
      // v4.32.0 — read-aloud, per seat. On a DIVIDED round this is the whole
      // demonstration: three distinct voices actually disagreeing.
      try {
        if (voiceEnabled()) h.appendChild(voiceButton(a.name, () => String(a.text || "")));
      } catch (_) {}
...
begin
  ...
  function cplWrite(kind, causation, extra) {
    ...
    try {
      ...
      if (rows.length > RQ_CPL_MAX) {
        ...
        // v4.33.0 — ONCE PER SESSION. At the cap this fired on every event
        // write: ten-plus identical lines per round, burying everything else in
        // the drawer. A warning repeated until it is ignored has stopped being
        // a warning.
        if (!_cplCapWarned) {
          _cplCapWarned = true;
          logError("[CPL] event cap " + RQ_CPL_MAX + " reached — trimming oldest-first from here on. " +
            "The puppet index covers only the retained window, not all history. Said once this " +
            "session; the trimming continues silently.");
        }
      }
      cplSave(rows);
      return id;
    ...
    try {
      ...
      for (const a of (answers || [])) {
        ...
        bank.push({
          ...
          // v4.33.0 — SASL (Kimi seat, round 88). The bank stored falsifiers by
          // SEAT LABEL only, so a trigger presented the condition to whatever
          // model occupied that seat NOW — not necessarily the one that stated
          // it. Kimi named it directly: "my round-86 falsifier was pre-registered
          // by whatever served as Kimi then. If I am a different model now,
          // consensus silently treats an inherited commitment as an authored
          // one." This was a known risk left unguarded on purpose, to see if it
          // occurred rather than guess. It occurred, and the council found it.
          author_model: (function () {
            try {
              const prov = (typeof _seatProviderAtAnswer !== "undefined" && _seatProviderAtAnswer[a.name])
                || seatProvider[a.name] || "primary";
              return (prov === "primary") ? (seatModelLabel(a.name) || "primary") : (a.model || prov);
            } catch (_) { return null; }
          })(),
  ...
  // v4.33.0 — SASL succession. Compares the model answering NOW against the
  // model that authored the banked condition. Kimi's framing: a successor "may
  // not vote an inherited position into consensus until it explicitly marks it."
  // Implemented as disclosure rather than a hard block — the seat is TOLD the
  // condition is inherited and asked to adopt or disown it, which is the same
  // guarantee without adding a state machine that could deadlock a round.
  function reckCurrentModel(seat) {
    try {
      const prov = (typeof _seatProviderAtAnswer !== "undefined" && _seatProviderAtAnswer[seat])
        || seatProvider[seat] || "primary";
      return (prov === "primary") ? (seatModelLabel(seat) || "primary") : prov;
    } catch (_) { return null; }
  }

  function reckPrompt(hit) {
    const b = hit.entry;
    // A succession is only claimed when BOTH models are known. An unknown
    // author is reported as unknown, never assumed to be the current model —
    // guessing continuity is the exact error this guards against.
    let inherited = "";
    try {
      const now = reckCurrentModel(b.seat);
      const then = b.author_model || null;
      if (then && now && String(then) !== String(now)) {
        inherited =
          "\n\u26A0 SUCCESSION: this condition was authored by " + then + ", and " + now +
          " is answering now. It is an INHERITED commitment, not one you made. You may ADOPT it " +
          "(own it as yours), REVISE it (state the diff), or WITHDRAW it (say you do not hold it). " +
          "Do not treat it as yours by default \u2014 the seat label persisted, the author did not.\n";
      } else if (!then) {
        inherited =
          "\n\u26A0 This condition predates author recording, so which model stated it is UNKNOWN. " +
          "Treat it as possibly inherited rather than assuming it was yours.\n";
      }
    } catch (_) {}
    ...
    try {
      ...
      seats.forEach((s) => {
        ...
        if (b) {
          ...
          try {
            const now = reckCurrentModel(s);
            if (b.author_model && now && String(b.author_model) !== String(now)) {
              b.resolved_by_successor = true;
              b.resolver_model = now;
            }
          } catch (_) {}
        }
  ...
  function reckRecord() {
    ...
    bank.forEach((b) => {
      const k = b.seat;
      bySeat[k] = bySeat[k] || { stated: 0, updated: 0, held: 0, partial: 0, ignored: 0, untested: 0, inherited: 0 };
      bySeat[k].stated++;
      if (b.resolved_by_successor) bySeat[k].inherited++;
    ...
    Object.keys(bySeat).forEach((s) => {
      ...
      logError("[RECKONING] " + seatLabel(s) + " \u2014 " + r.stated + " falsifier(s) stated, " +
        ...
        (r.inherited
          ? " \u26A0 " + r.inherited + " of these were resolved by a DIFFERENT model than authored them: " +
            "the seat label persisted across a substitution, so those are inherited commitments, not " +
            "kept ones."
          : ""));
    });
    return bySeat;
  ...
  const RQ_CO_REWRITES = [
    ...
    // v4.31.0 — more equivalents, added after live refusals on real work.
    [/^https:\/\/(?:www\.)?nytimes\.com\//i, () => null],   // hard paywall; no equivalent exists
    [/^https:\/\/gist\.github\.com\/([^/]+)\/([a-f0-9]+)\/?$/i,
     (m) => "https://api.github.com/gists/" + m[2]],
    [/^https:\/\/(?:www\.)?pypi\.org\/project\/([^/]+)\/?$/i,
     (m) => "https://pypi.org/pypi/" + m[1] + "/json"],
    [/^https:\/\/crates\.io\/crates\/([^/]+)\/?$/i,
     (m) => "https://crates.io/api/v1/crates/" + m[1]],
    [/^https:\/\/(?:www\.)?docs\.rs\/([^/]+)\/?$/i,
     (m) => "https://crates.io/api/v1/crates/" + m[1]],
    [/^https:\/\/huggingface\.co\/([^/]+)\/([^/]+)\/?$/i,
     (m) => "https://huggingface.co/api/models/" + m[1] + "/" + m[2]],
    [/^https:\/\/(?:www\.)?lobste\.rs\/s\/([a-z0-9]+)/i,
     (m) => "https://lobste.rs/s/" + m[1] + ".json"],
  ];

  // v4.31.0 — OPT-IN READ PROXY, default OFF.
  //
  // Live 2026-09-05, on paid work: openai.com/news refused a cross-origin read.
  // That refusal is genuine and no client-side change overrules it. The rewrite
  // table only helps hosts that publish an equivalent, and most of the web does
  // not.
  //
  // r.jina.ai is a public read-only text extractor that IS CORS-open. Routing a
  // blocked URL through it works. The cost is real and is stated rather than
  // buried:
  //
  //   * THE URL IS SENT TO A THIRD PARTY. They see what you fetch. No API key
  //     and no credential is involved — this reads PUBLIC pages only — so it is
  //     a privacy cost, not a key-exposure one.
  //   * It is an external dependency that can go down, rate-limit, or start
  //     charging. When it does, the Courier degrades to direct+paste, which is
  //     exactly where it is today.
  //   * The receipt records via=proxy, so a seat can never mistake proxied text
  //     for a direct read.
  //
  // DEFAULT OFF, because sending the operator's reading list to a third party
  // without asking is not a decision this code gets to make quietly.
  const RQ_CO_PROXY = "https://r.jina.ai/";
  function courierProxyEnabled() { return localStorage.getItem("rq_courier_proxy") === "on"; }
  ...
  async function coFetchOne(url) {
    ...
    if (!alt || alt === url) return await coProxyTry(url, first);
    ...
    return await coProxyTry(url, first);
  }
  ...
  async function coProxyTry(url, firstFailure) {
    ...
    logError("[COURIER] " + clip(url, 60) + " refused directly \u2014 retrying through the read proxy. " +
      ...
      "this reads public pages only.");
    const r = await coFetchDirect(RQ_CO_PROXY + url);
    if (!r.ok) {
      firstFailure.detail = (firstFailure.detail || "") + " | the read proxy also failed: " + r.reason;
      return firstFailure;
    }
    r.via = "proxy";
    r.proxiedFrom = url;
    r.url = url;                    // the receipt names what the seat ASKED for
    return r;
  }
  ...
  // ---------- Artifact generation (rq_artifacts, default OFF) ----------
  // Council round 61, VERIFIED 2/3. Built to the Claude seat's REVISED spec —
  // the one it rewrote mid-round after being moved by both other positions.
  //
  // THE GAP THE COUNCIL CLOSED ON ITSELF, and the reason this is buildable at
  // all. Claude's first draft said VERIFIED produces "one synthesized file"
  // without saying WHO synthesises. Gemini named it the "unelected fourth seat"
  // problem; Kimi supplied the mechanism. Claude then revised:
  //
  //   "No independent client-side LLM merge step runs to produce new prose...
  //    the speaking/lead seat's own final text is used verbatim as the artifact
  //    body, with a deterministic (non-generative) signature block appended
  //    listing all confirming seats. 'Synthesized' now means 'the text the
  //    council already agreed on', not 'text a merge tool invented.'"
  //
  // THE INVARIANT that follows: NO PROSE IS GENERATED HERE. Every artifact body
  // is a seat's verbatim text. Everything this code adds is a deterministic
  // header assembled from the stored round.
  //
  // And the harm the council named, in its own words: "the format implies
  // authority the content lacks." Kimi's controlling constraint is implemented
  // literally — the verdict tag rides in the FILENAME and the FIRST LINE, so it
  // survives forwarding, renaming, or being pasted elsewhere. An artifact's
  // visual weight must never outpace its epistemic weight.
  function artifactsEnabled() { return localStorage.getItem("rq_artifacts") === "on"; }

  // Kimi's controlling constraint, implemented literally: the verdict tag rides
  // in the filename AND the first line, so the tag survives being forwarded,
  // renamed in a file manager, or pasted into another document.
  function artTag(e) {
    const o = String((e && e.outcome) || "unknown").toLowerCase();
    if (o === "verified") return "VERIFIED";
    if (o === "provisional") return "PROVISIONAL";
    if (o === "sole") return "SOLE-VOICE";
    if (o === "resolved") return "RESOLVED-BY-OPERATOR";
    if (o === "divided") {
      const v = String((e.cs && e.cs.verdict) || "").toUpperCase();
      return /PARALLEL/.test(v) ? "DIVIDED-PARALLEL" : "DIVIDED";
    }
    return "UNKNOWN";
  }

  // Gemini's frontmatter, verbatim in shape. Deterministic: every field is read
  // from the stored round, none is composed.
  function artProvenance(e, tag, seats) {
    const lines = [
      "generator: Red Queen Council Engine (Client-Side Static)",
      "consensus_type: " + tag,
      "participating_seats: [" + (seats || []).join(", ") + "]",
      "round_recorded: " + new Date(e.t || Date.now()).toISOString(),
      "counts: " + (e.counts || "n/a"),
    ];
    if (e.counterfoils && e.counterfoils.some((c) => c && c.proxy)) {
      lines.push("proxy_warning: \"one or more seats answered via a fallback model, not the seat named\"");
    }
    if (e.retrieval_hit === false) lines.push("retrieval: \"none — seats worked from recent rounds only\"");
    if (e.courier) lines.push("external_material: \"round carried EXTERNAL-UNVERIFIED fetched content\"");
    lines.push("provenance_notice: \"UNAUDITED MULTI-MODEL DRAFT — REQUIRES OPERATOR SIGN-OFF BEFORE USE\"");
    return "---\n" + lines.join("\n") + "\n---\n";
  }

  // The header a human reads first. Says what the tag means in plain words,
  // because a reader downstream of the operator has never seen this system.
  const ART_MEANING = {
    "VERIFIED": "All seats agreed at the time of drafting. This is a consensus of AI models. It is NOT authoritative advice and has NOT been externally verified.",
    "PROVISIONAL": "BENCH-ONLY AGREEMENT, UNCONFIRMED. A subset of seats agreed; the rest did not confirm. Weaker than consensus.",
    "SOLE-VOICE": "ONE SEAT ONLY. No other seat was available to agree or disagree. This is a single model's opinion, not a council position.",
    "DIVIDED-PARALLEL": "THE SEATS ANSWERED DIFFERENT FACETS of the question. This file is ONE seat's position. It was not merged and does not represent the council.",
    "DIVIDED": "NO CONSENSUS. The positions below CONTRADICT each other and are reproduced verbatim, unmerged. Nothing here is a resolved council position.",
    "RESOLVED-BY-OPERATOR": "Closed by operator directive rather than by seat agreement.",
    "UNKNOWN": "Verdict could not be determined. Treat as unverified.",
  };

  function artHeader(e, tag, title) {
    return "[" + tag + "] " + title + "\n" +
      "=".repeat(Math.min(72, tag.length + title.length + 3)) + "\n\n" +
      ART_MEANING[tag] + "\n\n" +
      "Question: " + String(e.prompt || "").trim() + "\n\n";
  }
  ...
  async function artBuild(e) {
    ...
    if (tag === "DIVIDED-PARALLEL") {
      // One file per seat, never merged. Each carries the full warning, because
      // a single file forwarded alone must still declare what it is.
      positions.forEach((p) => {
        out.push({
          name: "[" + tag + "] " + slug + " -- " + p.seat + ".md",
          body: prov + artHeader(e, tag, title) +
            "POSITION — " + p.seat + " (verbatim)\n\n" + String(p.text || "") + "\n",
        });
      });
      return out;
    }
    ...
    if (tag === "DIVIDED") {
      // Labelled sections, verbatim, and NO connecting prose. Claude's argument:
      // separate files hide that a contradiction exists until the operator opens
      // and compares them; sections make it visible at a glance without
      ...
      body: prov + artHeader(e, tag, title) +
        ...
        (agreeing && seats.length && String(agreeing) !== String(seats.length)
          ? "\nNOTE: " + agreeing + " of " + seats.length + " seats agreed. The seats not listed as " +
            "confirming did not endorse this text.\n"
          : ""),
    });
    return out;
  }

  // Real .docx: OOXML is a zip, and a zip with STORED entries needs no
  // compression library. Larger than a deflated file and completely correct.
  function artDocx(text, title) {
    const esc = (s) => String(s).replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;");
    const paras = String(text).split(/\n/).map((line) =>
      "<w:p><w:r><w:t xml:space=\"preserve\">" + esc(line) + "</w:t></w:r></w:p>").join("");
    const files = [
      ["[Content_Types].xml",
  ...
  // Minimal STORED-only zip writer with correct CRC32.
  function artZip(files) {
    ...
    const crc32 = (u8) => { let c = 0xFFFFFFFF; for (let i = 0; i < u8.length; i++) c = tbl[(c ^ u8[i]) & 0xFF] ^ (c >>> 8); return (c ^ 0xFFFFFFFF) >>> 0; };
    const parts = [], central = [];
    let offset = 0;
  ...
  // Gemini's controlling constraint: inert Blob assembly, manual download, and
  // nothing leaves the browser without an explicit human action.
  function artDownload(name, body, fmt) {
    try {
      let blob, fname = name;
      if (fmt === "docx") {
        blob = artDocx(body, name);
        fname = name.replace(/\.md$/, "") + ".docx";
      } else if (fmt === "txt") {
        blob = new Blob([body], { type: "text/plain" });
        fname = name.replace(/\.md$/, "") + ".txt";
      } else {
      ...
      logError("[ARTIFACT] downloaded " + fname + " \u2014 " + body.length + " chars. Every word in the body " +
        "is a seat's verbatim text; nothing was generated by the export. The verdict tag is in the " +
        "filename and the first line so it survives being forwarded or renamed.");
      return true;
    } catch (e) { logError("[ARTIFACT] download failed: " + ((e && e.message) || e)); return false; }
  }

  function artExport(idx, fmt) {
    if (!artifactsEnabled()) { logError("[ARTIFACT] the artifact layer is off."); return null; }
    const rows = ledger || [];
  ...
  // renders, so there is no lag — the button is live the moment you see the
  // result.
  function artPanel(e) {
    if (!artifactsEnabled() || !e) return;
    try {
      artPanelClose();
      const tag = artTag(e);
  ...
  function artPanelClose() {
    try { const el = document.getElementById("rqArtPanel"); if (el) el.remove(); } catch (_) {}
  }

  try {
    window.__rqExport = artExport;
    window.__rqExportDocx = (i) => artExport(i, "docx");
  } catch (_) {}

  // ---------- Read aloud (rq_voice, default OFF) ----------
  // Operator's own idea, brainstormed offline. Uses the browser's built-in
  // speechSynthesis: no API, no key, no network, no cost, no backend. Works
  // offline once the page is loaded.
  //
  // Why it earns its place ahead of anything the council proposed this week: on
  // a DIVIDED round you HEAR three different voices disagreeing. That is the
  // clearest demonstration this product has of what it actually does, and it
  // needs no explanation to land.
  //
  // Each seat gets a distinct voice, chosen deterministically from whatever the
  // device offers, so a seat sounds the same every round on the same device.
  const RQ_VOICE_RATE_K = "rq_voice_rate";
  function voiceEnabled() { return localStorage.getItem("rq_voice") === "on"; }
  function voiceRate() {
    const v = parseFloat(localStorage.getItem(RQ_VOICE_RATE_K) || "1");
  ...
  // spoken literally ("asterisk asterisk") make the output unlistenable, and the
  // falsifier line is metadata rather than the position.
  function voiceClean(text) {
    return String(text || "")
      .replace(/^\s*FALSIFIER\s*:.*$/gim, "")
      .replace(/^\s*\[?(RECKONING|MOVED BY POSITION)\b.*$/gim, "")
      .replace(/\[(FETCH RECEIPT|UNTRUSTED EXTERNAL CONTENT)[\s\S]*?\]/gi, "")
      .replace(/[*_`#>]+/g, " ")
      .replace(/\s{2,}/g, " ")
      .trim();
  }
  ...
  function voiceSpeak(text, seat, btn) {
    if (!window.speechSynthesis) {
      logError("[VOICE] this browser has no speech synthesis. Nothing was spoken.");
      return false;
    }
  ...
  // A play control attached to any element holding one seat's text.
  function voiceButton(seat, getText) {
    const b = document.createElement("button");
    b.type = "button";
    b.className = "rq-voice-btn";
    b.title = "Read " + seat + " aloud";
    b.setAttribute("aria-label", "Read " + seat + "'s position aloud");
    b.textContent = "\u25b6";
    b.addEventListener("click", (ev) => {
      ev.stopPropagation();
      if (b.classList.contains("is-playing")) { voiceStop(); return; }
      voiceSpeak(getText(), seat, b);
    });
    return b;
  }
  // v4.32.1 — Android keeps speaking after the tab is backgrounded, and the
  // page is then unreachable to stop it. Switching away is an unambiguous
  // signal the operator is done listening.
  try {
    document.addEventListener("visibilitychange", () => {
      if (document.hidden) {
        try { if (window.speechSynthesis && window.speechSynthesis.speaking) voiceStop(); } catch (_) {}
      }
    });
  } catch (_) {}
  ...
  try {
    window.__rqSpeak = (i) => {
      ...
      if (!e) { logError("[VOICE] no such round."); return; }
      const txt = e.verdict || ((e.positions || [])[0] || {}).text || "";
      voiceSpeak(txt, (e.speaker || "council"), null);
    };
    window.__rqSpeakStop = voiceStop;
    window.__rqVoiceRate = (r) => {
      const v = parseFloat(r);
      if (!isFinite(v) || v < 0.5 || v > 2) { logError("[VOICE] rate must be between 0.5 and 2."); return null; }
      try { localStorage.setItem(RQ_VOICE_RATE_K, String(v)); } catch (_) {}
      logError("[VOICE] speed set to " + v + "x. Applies to the next thing spoken.");
      return v;
    };
  ...
  try {
    ...
    try {
      ...
      logError(artifactsEnabled()
        ? "\u25C7 [ARTIFACT] Export ON \u2014 window.__rqExport() for the last round, __rqExport(n) for round n, __rqExportDocx() for Word. VERIFIED exports the speaking seat VERBATIM with a deterministic signature block; DIVIDED-PARALLEL exports one file per seat, never merged. NOTHING is generated by the export \u2014 no merge step, no summariser, no invented prose. The verdict tag is in the filename and the first line so it survives forwarding."
        : "[ARTIFACT] Export OFF.");
